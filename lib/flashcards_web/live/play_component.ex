defmodule FlashcardsWeb.PlayComponent do
  ...
  def update(assigns, socket) do
    ...
    test_id = if is_nil(test_id) and not is_nil(selected_group_id), do: create_test(user_id, selected_group_id), else: test_id
    socket =
      socket
      |> assign(assigns)
  ...
  defp create_test(user_id, group_id) do
    attrs = %{user_id: user_id, group_id: group_id, know_count: 0, dont_know_count: 0, show_count: 0}
    result = Flashcards.Flashcards.Test |> Ash.Changeset.for_create(:create, attrs) |> Ash.create()
  ...
  defp create_score(test_id, card_id, know, dont_know, status) do
    attrs = %{test_id: test_id, card_id: card_id, know: know, dont_know: dont_know, status: status}
    result = Flashcards.Flashcards.Score |> Ash.Changeset.for_create(:create, attrs) |> Ash.create()
    ...
    <div class="w-full max-w-md mx-auto mt-10 bg-white p-8 rounded-xl shadow-lg">
      ...
      <form phx-change="select_group" phx-target={@myself}>
        ...
        <select name="group_id" id="group_id" class="rounded border-gray-300 px-2 py-1 text-sm focus:ring-blue-400 focus:border-blue-400">
          <option value="" disabled selected={is_nil(@selected_group_id) or @selected_group_id == ""}>Choose Group</option>
      ...
      <% else %>
        <%= if is_nil(@selected_group_id) or @selected_group_id == "" do %>
          <div class="mt-8 text-center text-lg text-red-600 font-semibold">Please choose a group to start a session.</div>
        <% else %>
          <div class="mt-8 flex flex-col items-center">
            ...
            <% else %>
              ...
              <%= if @test_id do %>
                ...
                <% percent_known = if total > 0, do: Float.round(test.know_count / total * 100, 1), else: 0 %>
                <% percent_dont_know = if total > 0, do: Float.round(test.dont_know_count / total * 100, 1), else: 0 %>
                <div class="mt-6 p-6 rounded-lg bg-green-50 shadow text-center">
                  ...
                  <div class="text-base text-green-900 mb-1">Known: <%= test.know_count %> (<%= percent_known %>%)</div>
                  <div class="text-base text-yellow-900 mb-1">Don't Know: <%= test.dont_know_count %> (<%= percent_dont_know %>%)</div>
                  <div class="text-base text-blue-900 mb-1">Show Answer: <%= test.show_count %></div>
                  <button type="button" phx-click="restart_test" phx-target={@myself} class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Restart Session</button>
                </div>
                <% dont_know_scores = Flashcards.Flashcards.Score |> Ash.Query.filter(expr(test_id == ^@test_id and dont_know == true)) |> Ash.read!() %>
                  ...
                  <div class="mt-8 p-6 rounded-lg bg-yellow-50 shadow">
                    ...
                    <ul class="list-disc list-inside text-left">
                      <%= for score <- dont_know_scores do %>
                        <% card = Flashcards.Flashcards.Flashcard |> Ash.Query.filter(expr(id == ^score.card_id)) |> Ash.read_one!() %>
                        <li class="mb-2">
