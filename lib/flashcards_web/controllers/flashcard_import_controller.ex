defmodule FlashcardsWeb.FlashcardImportController do
  ...
  def create(conn, params) do
    if conn.assigns[:current_user] do
      ...
      cond do
        !user_id ->
          conn
          |> put_flash(:error, "You must be signed in to import flashcards.")
        ...
        group_name == "" ->
          conn
          |> put_flash(:error, "Please provide a group name.")
          |> redirect(to: ~p"/flashcards/import")
            ...
            {:ok, group} ->
              ...
              case result do
                {:ok, count} ->
                  ...
                  |> put_flash(
                    :info,
                    "Successfully imported #{count} flashcards to group '#{group_name}'."
                  )
  ...
  defp parse_and_import(%Plug.Upload{path: path}, overwrite, group_id, user_id) do
    ...
    with {:ok, json} <- File.read(path),
      ...
      flashcards =
        Enum.map(data, fn %{"question" => q, "answer" => a} ->
      ...
      error ->
        IO.inspect(error, label: "IMPORT ERROR")
        {:error, "Invalid JSON format. Must be a list of %{question, answer}"}
    end
  end
end
